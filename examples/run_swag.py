...
def read_swag_examples(input_file, is_training):
    ...
    examples = [
        SwagExample(
            ...
            label = row['label'] if is_training else 0
...
def predict_on_test(model,test_dataloader,args,best_eval_acc):
    model.eval()
    logits_all=None
    ...
    f=open(os.path.join(args.data_dir, f'test_answer_{best_eval_acc}_{args.do_margin_loss}.csv'),'w')
    f.write('pred\n')
...
def main():
    ...
    if args.do_train:
        train_examples = read_swag_examples(os.path.join(args.data_dir, 'train_small.csv'), is_training = True)
        num_train_steps = int(
    ...
    if args.do_margin_loss==0:
        ...
        model = BertForMultipleChoiceMarginLoss.from_pretrained(args.bert_model,
            cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(args.local_rank),
            num_choices = 4, margin=args.margin
        )
    if args.fp16:
        model.half()
    model.cuda()
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        test_examples = read_swag_examples(os.path.join(args.data_dir, 'test.csv'), is_training = False)
    ...
    # print('Best eval acc:',best_eval_acc)
            ...
            if best_eval_acc<eval_accuracy:
                ...
                print(best_eval_acc)
                # predict_on_test(model,test_dataloader,args,best_eval_acc)
        print(best_eval_acc,best_step,args.seed,args.margin,args.do_margin_loss,args.bert_model,sys.argv[0])
...
#     if max_length<len1:
#         max_length=len1
#     len2=len((str(start_phrase)+str(ending_2)).split())
#     if max_length<len2:
#         max_length=len2
#     len3=len((str(start_phrase)+str(ending_3)).split())
#     if max_length<len3:
#         max_length=len3
