...
def read_roc_examples(input_file, is_training):
    ...
    if is_training and 'AnswerRightEnding' not in input_df.columns:
        raise ValueError(
            "For training, the input file must contain a label column.")

    examples = [
        RocExample(
            InputStoryid = row['InputStoryid'],
            # context_sentence = ' '.join([row['InputSentence1'],row['InputSentence2'],row['InputSentence3'],row['InputSentence4']]),
            context_sentence='',
            # context_sentence = ' '.join([row['InputSentence2'],row['InputSentence3'],row['InputSentence4']]),
            RandomFifthSentenceQuiz1=row['RandomFifthSentenceQuiz1'],
            RandomFifthSentenceQuiz2=row['RandomFifthSentenceQuiz2'],
            AnswerRightEnding = row['AnswerRightEnding'] if is_training else 0
...
def predict_on_test(model,eval_dataloader,args,best_test_acc):
    output_csv=pd.read_csv(os.path.join(args.data_dir, 'output_test.csv'))
    model.eval()
    logits_all=None
    ...
    output_csv.to_csv(os.path.join(args.data_dir, f'answer_test_{best_test_acc}_{args.do_margin_loss}_{args.margin}_{args.seed}_{model_name}.txt'),index=False,float_format='%.f')
...
def main(MARGIN=0.15):
    ...
    if args.do_train:
        ...
        # train_examples = read_roc_examples(os.path.join(args.data_dir, 'test.csv'), is_training = True)
        ...
        if args.do_eval:
            eval_examples=train_examples[0:100]
    ...
    if args.do_margin_loss==0:
        ...
        model = BertForMultipleChoiceMarginLoss.from_pretrained(args.bert_model,
            cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(args.local_rank),
            num_choices = 2, margin=args.margin
        )
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        # test_examples = read_roc_examples(os.path.join(args.data_dir, 'testnew.csv'), is_training = False)
        test_examples = read_roc_examples(os.path.join(args.data_dir, 'test.csv'), is_training = True)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if epoch>0:
                        ...
                        if best_eval_acc<eval_accuracy:
                            ...
                            logits_all,best_test_acc=do_evaluation(model,test_dataloader,args,is_training=False)
                            print(best_test_acc,best_eval_acc)
                            if args.do_prediction:
                                predict_on_test(model,test_dataloader,args,best_test_acc)
                            # if best_test_acc>0.90058:
                            #     torch.save(model.state_dict(), os.path.join(args.output_dir, f"pytorch_model.bin"))
                            #     return
        print(best_test_acc,best_eval_acc,best_step,args.seed,args.margin,args.do_margin_loss,args.learning_rate,args.bert_model,sys.argv[0])
...
# input_df = pd.read_csv('val.csv')
# length_list=[]
# for _, row in input_df.iterrows():
#     max_length=0
#     start_0 = row['InputSentence1'],
#     start_1 = row['InputSentence2'],
#     start_2 = row['InputSentence3'],
#     start_3 = row['InputSentence4'],
#     ending_0 = row['RandomFifthSentenceQuiz1'],
#     ending_1 = row['RandomFifthSentenceQuiz2'],
#     start_phrase=str(start_0)+' '+str(start_1)+' '+str(start_2)+' '+str(start_3)
#     len0=len((str(start_phrase)+' '+str(ending_0)).split())
#     if max_length<len0:
#         max_length=len0
#     len1=len((str(start_phrase)+' '+str(ending_1)).split())
#     if max_length<len1:
#         max_length=len1
